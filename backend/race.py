...
class Race(gym.Env):
    ...
    def render(self, mode=None):
        ...
        def init_renderer(self):
            ...
            self.FERRARI_IMG = utils.scale_image(pygame.image.load('ferrari.png').convert_alpha(), 0.1)
            self.REDBULL_IMG = utils.scale_image(pygame.image.load('redbull.png').convert_alpha(), 0.1)
            self.MERCEDES_IMG = utils.scale_image(pygame.image.load('mercedes.png').convert_alpha(), 0.1)
            self.CAR_IMG = random.choice([self.FERRARI_IMG, self.REDBULL_IMG, self.MERCEDES_IMG])
            self.CAR_IMG_OFF = utils.scale_image(pygame.image.load('ferrari_off.png').convert_alpha(), 0.1)
            self.BG_IMG = utils.scale_image(pygame.image.load('Monza_background.png').convert_alpha(), 1/1.3)
            self.TRACK_IMG = utils.scale_image(pygame.image.load('Monza_track_extra_wide_2.png').convert_alpha(), 1/1.3)
