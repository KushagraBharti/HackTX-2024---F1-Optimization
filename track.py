...
'''
    def show_track(self):
        ...
        plt.plot(self.line1[:, 0], self.line1[:, 1], label='Outer Boundary', color='blue')
        plt.plot(self.line2[:, 0], self.line2[:, 1], label='Inner Boundary', color='red')
        
        # Plot goals
        for goal in self.goals:
            plt.plot([goal[0], goal[2]], [goal[1], goal[3]], 'g--')

        plt.gca().invert_yaxis()
        plt.legend()
        plt.title("Track Display")
        plt.show()

    

# Instantiate and display
track = Track(game=None)  # Pass your actual game instance if needed
track.show_track()
